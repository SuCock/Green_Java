...
export const ShopModal = ({ onCloseModal }) => {
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            console.log("요청결과 = " + JSON.stringify(response.data));
        }
        getData();
    ...
    const drawMap = async () => {
        const resultData = await getAddress();
        ...
        function calculateDistancesFromCurrentLocation(currentLat, currentLon, locations) {
            for (let i = 0; i < shopList.length; i++) {
                ...
                distances.push(calculateDistance(location.location_x, location.location_y, resultData[i].Ma, resultData[i].La));
            }
            return distances;
        }

        var distancess = calculateDistancesFromCurrentLocation(location.location_x, location.location_y, resultData); // 리스트 안에 리스트를 담아서 배열형태가 아니라서 오류가 났던거다
